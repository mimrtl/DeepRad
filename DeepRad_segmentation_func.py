...
from DeepRad_segmentation import Ui_MainWindow
from configuration import Config_Segmentation
from include.thread import SegmentationTrain
import dataAug
import LossConfig
import OptimizerConfig
from visualization import VisualizationMainWindow, VisualizationDialog
...
class mainwindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def setupMain(self):
        self.loss_list = ["dice loss", "cross entropy", "weighted dice loss"]
        ...
        self.config_segmentation = Config_Segmentation()
    ...
    def isValidConfig(self):
        self.config_segmentation.configCheck()
        return self.config_segmentation.is_valid_config

    def transformConfig(self):
        self.config_segmentation.configTransform()
    ...
    def addObjectAndThreadToNewItem(self):
        self.thread_dict[str(self.new_thread_index)].append(
            SegmentationTrain(self.new_thread_index, self.config_segmentation)
        )
        self.thread_dict[str(self.new_thread_index)].append(
            QtCore.QThread()
        )
    ...
    def processMessageFromTrainingThread(self, message):
        ...
        self.testGetStatusInfo(message)
    ...
    def getAllMessageString(self):
        return listOfStr2Str(self.config_segmentation.check_result, seperator='\n')
    ...
    def getCurrentPrepareDataConfigFromGUI(self):
        ...
        self.config_segmentation.config['is_resize'] = self.checkBox_9.isChecked()
        self.config_segmentation.config['resize_row'] = self.lineEdit_18.text()
        self.config_segmentation.config['resize_col'] = self.lineEdit_19.text()
        self.config_segmentation.config['resize_channel'] = self.lineEdit_20.text()

    def getCurrentChooseModelConfigFromGUI(self):
        self.config_segmentation.config['model'] = self.comboBox.currentText()
        self.config_segmentation.config['input_size_row'] = self.lineEdit_4.text()
        self.config_segmentation.config['input_size_col'] = self.lineEdit_5.text()
        self.config_segmentation.config['input_size_channel'] = self.lineEdit_6.text()
        self.config_segmentation.config['num_class'] = self.lineEdit_7.text()

    def getCurrentDataAugConfigFromGUI(self):
        self.config_segmentation.config['isDataAug'] = self.checkBox.isChecked()
        self.config_segmentation.config['data_aug_config'] = self.DataAugConfig.DataAugConfig

    def getCurrentTrainingConfigFromGUI(self):
        self.config_segmentation.config['learning_rate'] = self.lineEdit.text()
        self.config_segmentation.config['drop_factor'] = self.lineEdit_14.text()
        self.config_segmentation.config['patience'] = self.lineEdit_15.text()
        self.config_segmentation.config['batch_size_training'] = self.lineEdit_2.text()
        self.config_segmentation.config['batch_size_validation'] = self.lineEdit_16.text()
        self.config_segmentation.config['epoch'] = self.lineEdit_3.text()
        self.config_segmentation.config['early_stop'] = self.lineEdit_17.text()

        self.config_segmentation.config['LossConfig'] = self.LossConfig.LossConfig
        self.config_segmentation.config['OptimizerConfig'] = self.OptimizerConfig.OptimizerConfig

    def getCurrentOutputConfigFromGUI(self):
        self.config_segmentation.config['output_folder'] = self.lineEdit_9.text()
        self.config_segmentation.config['is_file_only'] = self.radioButton_5.isChecked()
        self.config_segmentation.config['isWeight'] = self.checkBox_2.isChecked()
        self.config_segmentation.config['isTensorboard'] = self.checkBox_3.isChecked()
        self.config_segmentation.config['isLogs'] = self.checkBox_4.isChecked()
